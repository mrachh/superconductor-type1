...
cc      dzk = 0.19d0
     ...
     1   nn*fjvals(nn+1)*fhvals(nn)-zk*fjvals(nn+1)*fhvals(nn-1))
      ...
      zdelta =  (fhvals(nn)+zk*fhder(nn))*fjvals(nn)*zk*ima
      ddeltam = real(zdelta)

      zdelta = ima*(fjvals(nn)+zk*fjder(nn))*fhvals(nn)*zk
      ddeltap = real(zdelta)
      ddelta = 0.5d0*(ddeltap + ddeltam) 
...
C$OMP END PARALLEL DO    
     ...
     1      iquad,rfac0,nquad,wnear)
      ...
      call lpcomp_statj_gendeb_postproc_addsub(npatches,norders,ixyzs,
     1  iptype,
     1  npts,srccoefs,srcvals,eps,dzk,nnz,row_ptr,col_ind,iquad,nquad,
     2  wnear,ngenus,hvecs,bbphvecs,sigma,novers,npts_over,ixyzso,
     3  srcover,wover,bjm,bbm,bbp)

      rjmrmumn = deta*dzk**2*sqrt((nn+0.0d0)*(nn+1.0d0))/(2*nn+1.0d0)*
     1  rcrmum
      rjmrmumtu = dalpha*dzk**2*
     1  sqrt((nn+0.0d0)*(nn+1.0d0))/(2*nn+1.0d0)*rcrmum 
      rjmrmumtx = 0

      rjmrhomn = 0*rcrhom
      rjmrhomtx = ddeltam*dzk*
     1  sqrt((nn+0.0d0)*(nn+1.0d0))/(2*nn+1.0d0)*rcrhom
      rjmrhomtu = 0

      rjmrhopn = dbeta*rcrhop/(2*nn+1.0d0)*(nn+0.0d0)*(nn+1.0d0)*dzk
      rjmrhoptu = -ddeltap*dzk*
     1  sqrt((nn+0.0d0)*(nn+1.0d0))/(2*nn+1.0d0)*rcrhop
      rjmrhoptx = 0
      ...
      rjmtu = rjmrhoptu + rjmrhomtu + rjmrmumtu + rjmrmtu
      rjmtx = rjmrhoptx + rjmrhomtx + rjmrmumtx + rjmrmtx
      rjmn = rjmrhopn + rjmrhomn + rjmrmumn + rjmrmn

      print *, "rjmrmumn=",rjmrmumn
      ra = 0
      erra = 0
      do i=1,npts
        wtmp1(1:3) = rjmn*rrhs(i)*srcvals(10:12,i) + 
     1     rjmtu*unm(1:3,i) + rjmtx*xnm(1:3,i)
        erra = erra + abs(bjm(1,i)-wtmp1(1))**2*wts(i)
        erra = erra + abs(bjm(2,i)-wtmp1(2))**2*wts(i)
        erra = erra + abs(bjm(3,i)-wtmp1(3))**2*wts(i)
        ra = ra + abs(rrhs(i))**2*wts(i)
      enddo
      erra = sqrt(erra/ra)
      call prin2('error in j=*',erra,1)

      rbmrmumn = 0*rcmum
      rbmrmumtx = ddeltam*dzk*
     1  sqrt((nn+0.0d0)*(nn+1.0d0))/(2*nn+1.0d0)*rcrmum
      rbmrmumtu = 0

      rbmrhomn = -deta*dzk**2*sqrt((nn+0.0d0)*(nn+1.0d0))/(2*nn+1.0d0)*
     1  rcrhom
      rbmrhomtu = -dalpha*dzk**2*
     1  sqrt((nn+0.0d0)*(nn+1.0d0))/(2*nn+1.0d0)*rcrhom 
      rbmrhomtx = 0
     ...
     1  sqrt((nn+0.0d0)*(nn+1.0d0))/(2*nn+1.0d0)*rcrhop
      ...
      rbmn = rbmrhopn + rbmrmumn + rbmrhomn + rbmqmn
      rbmtu = rbmrmumtu + rbmrhomtu + rbmrhoptu + rbmqmtu
      rbmtx = rbmrmumtx + rbmrhomtx + rbmrhoptx + rbmqmtx

      ra = 0
      erra = 0
      do i=1,npts
        wtmp1(1:3) = rbmn*rrhs(i)*srcvals(10:12,i) +
     1     rbmtu*unm(1:3,i) + rbmtx*xnm(1:3,i)
        erra = erra + abs(bbm(1,i)-wtmp1(1))**2*wts(i)
        erra = erra + abs(bbm(2,i)-wtmp1(2))**2*wts(i)
        erra = erra + abs(bbm(3,i)-wtmp1(3))**2*wts(i)
        ra = ra + abs(rrhs(i))**2*wts(i)
      enddo
      erra = sqrt(erra/ra)
      call prin2('error in bm=*',erra,1)


      rbpn = -(nn+1.0d0)/(2*nn+1.0d0)*rcqp
      rbptu = sqrt((nn+0.0d0)*(nn+1.0d0))/(2*nn+1.0d0)*rcqp
      rbptx = 0

      erra = 0
      ra = 0
      do i=1,npts
        wtmp1(1:3) = rbpn*rrhs(i)*srcvals(10:12,i) + 
     1    rbptu*unm(1:3,i) + rbptx*xnm(1:3,i)
        erra = erra + abs(bbp(1,i)-wtmp1(1))**2*wts(i)
        erra = erra + abs(bbp(2,i)-wtmp1(2))**2*wts(i)
        erra = erra + abs(bbp(3,i)-wtmp1(3))**2*wts(i)
        ra = ra + abs(rrhs(i))**2*wts(i)
      enddo

      erra = sqrt(erra/ra)
      call prin2('error in bp = *',erra,1)
