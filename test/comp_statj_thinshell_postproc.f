...
c       
     ...
     1        iptype2(npatches2))
      ...
      if(idzk.lt.10) then 
        write(dirname, '(a,i1,a,i1,a,i1,a,i1)') 
     1   'static_curr_results/ap_',
     1   ifa,'_bm_',ifb,'_iref_',iref,'_idzk_',idzk
      else
        write(dirname, '(a,i1,a,i1,a,i1,a,i2)') 
     1   'static_curr_results/ap_',
     1   ifa,'_bm_',ifb,'_iref_',iref,'_idzk_',idzk
      endif
      print *, trim(dirname)

      allocate(bbmcomp(3,npts),bjmcomp(3,npts),bbpcomp(3,npts))

      fname = trim(dirname)//'/bbmcomp.dat'
      open(unit=77,file=trim(fname))
      do i=1,npts
         read(77,*) bbmcomp(1,i), bbmcomp(2,i), bbmcomp(3,i)
      enddo
      close(77)
      
      fname = trim(dirname)//'/bbm_outer.vtk'
      call surf_vtk_plot_vec(npatches1, norders1, ixyzs1, iptype1, 
     1  npts1, srccoefs1, srcvals1, bbmcomp, trim(fname), 'a')

      fname = trim(dirname)//'/bbm_inner.vtk'
      call surf_vtk_plot_vec(npatches2, norders2, ixyzs2, iptype2, 
     1  npts2, srccoefs2, srcvals2, bbmcomp(1,npts1+1), trim(fname),'a')

      fname = trim(dirname)//'/bjmcomp.dat'
      open(unit=77,file=trim(fname))
      do i=1,npts
         read(77,*) bjmcomp(1,i), bjmcomp(2,i), bjmcomp(3,i)
      enddo
      close(77)
      
      fname = trim(dirname)//'/bjm_outer.vtk'
      call surf_vtk_plot_vec(npatches1, norders1, ixyzs1, iptype1, 
     1  npts1, srccoefs1, srcvals1, bjmcomp, trim(fname), 'a')

      fname = trim(dirname)//'/bjm_inner.vtk'
      call surf_vtk_plot_vec(npatches2, norders2, ixyzs2, iptype2, 
     1  npts2, srccoefs2, srcvals2, bjmcomp(1,npts1+1), trim(fname),'a')
