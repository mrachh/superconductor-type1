     ...
     2   nb,bvals,bwts,bpatches,buv)
      ...
      allocate(hvecs(3,npts,2),bbphvecs(3,npts,2))

      do i=1,npts
        rr1 = srcvals(1,i)**2 + srcvals(2,i)**2
        hvecs(1:3,i,1) = srcvals(4:6,i)/rr1
...
C$OMP END PARALLEL DO    
     ...
     2    wnear(2*nquad+1),hvecs(1,1,i),novers,npts_over,ixyzso,srcover,
     3    wover,bbphvecs(1,1,i))
        do j=1,npts
          call cross_prod3d(srcvals(10,i),hvecs(1,j,i),vtmp1)
          bbphvecs(1:3,j,i) = bbphvecs(1:3,j,i) - vtmp1(1:3)/2
          write(47+i,'3(2x,e11.5)') bbphvecs(1,j,i),bbphvecs(2,j,i),
     1       bbphvecs(3,j,i)
        enddo
      enddo
