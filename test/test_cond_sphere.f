      ...
      do i=0,nmax
        call get_ynm_mat(i,amat(1,1,i),bmat(1,1,i))
        print *, bmat(2,2,0)
        call prin2('amat=*',amat(1,1,i),36)
        call prin2('bmat=*',bmat(1,1,i),9)
        write(77,*) amat(6,2,i),bmat(3,2,i)
     ...
     1     wtmp,1,wtmp,1,work,lw,info)
        ...
        write(35,'(6(2x,e11.5))') aeigi(1:6,i)
        write(36,'(3(2x,e11.5))') beigr(1:3,i)
        write(37,'(3(2x,e11.5))') beigi(1:3,i)
      enddo
      ...
      subroutine get_ynm_mat(nn,amat,bmat)

      implicit real *8 (a-h,o-z) 
      real *8 amat(6,6),bmat(3,3)
...
cc      call prin2('ztetam=*',ztetam,2)
      ...
      rinv1 = -(2*nn+1.0d0)/(nn+0.0d0)/(nn+1.0d0)

      if(nn.eq.0) rinv1 = 1.0d0
      ...
      amat(4,1) = dzk*(nn+0.0d0)*(nn+1.0d0)*dalpha/(2*nn+1.0d0)**2
      amat(4,2) = 0
      amat(4,3) = 0
      ...
      amat(5,1) = -dzk*sqrt(nn*(nn+1.0d0))*deta/(2*nn+1.0d0)
      amat(5,2) = 0
      amat(5,3) = 0
      amat(5,4) = dtetam/dzk
      ...
      amat(6,1) = 0
      amat(6,2) = dbeta*dzk*nn*(nn+1.0d0)/(2*nn+1.0d0)
